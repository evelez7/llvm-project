...
Error MDMustacheGenerator::setupTemplateFiles(const ClangDocContext &CDCtx) {
  ...
  std::string CommentFilePath = CDCtx.MustacheTemplates.lookup("comment");
  std::string FunctionFilePath = CDCtx.MustacheTemplates.lookup("function");
  std::vector<std::pair<StringRef, StringRef>> Partials = {
      {"Comments", CommentFilePath}, {"Function", FunctionFilePath}};
...
Error MDMustacheGenerator::generateDocumentation(
  ...
  return MustacheGenerator::generateDocumentation(RootDir, std::move(Infos), CDCtx, "md");
}
...
static GeneratorRegistry::Add<MDMustacheGenerator>
    MDHTML(MDMustacheGenerator::Format,
